...
point call(point a,point b,point c){
    point low = a,high = b;
    point z;
    line l = line(a,b);
    int ts = 100;
    while(ts--){
        point mid = low.add(high);
        mid = mid.div(2);
        double dd = c.distance(a) + mid.distance(a);
        double d = c.distance(b) + mid.distance(b);
        if(EQ(dd-d)>=0) high = mid;
        else low = mid;
    }
    return low;
}
...
int main() {
    ...
    For(cs,t){
        ...
        line l3 = line(b,ans);
        line l4 = line(a,c);
        point x = l3.crosspoint(l4);
        double dd = b.distance(c) + c.distance(x);
        double d = b.distance(a) + a.distance(x);
        OC;
        if(fabs(dd-d) < 1e-6)cout <<" "<< ans.x+EPS << ' ' << ans.y+EPS << endl;
        else printf(" ERROR\n");
    }
}
